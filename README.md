...
with the `href` attribute pointed to `index.html`. We then want to include the
visible text that will become the clickable text on our page, so let's add in
'About' in between the opening and closing tags. Go ahead and add an additional
`<a>` tag pointed to our 'New Properties,' 'Listings,' 'Market Report,' and
'Contact' HTML pages.
...
path. Since we're building real estate website, we may want to provide a link
...
current tab to change location. We've now got 6 links:
...
our pages haven't been touched yet, so you may need to add some HTML and
